...
| `GROQ_API_KEY` | `sk-***` | Chave Groq |
| `GROQ_MODEL` | `llama-3.3-70b-versatile` | Modelo padrão de IA |
...
> Em produção (Swarm), os segredos sensíveis (`SECRET_KEY`, senhas, `GROQ_API_KEY`) podem ser geridos por **Docker secrets/configs** em vez de `.env` em texto plano (ver seção 44).
...
cp .env.example .env
nano .env   # preencher SECRET_KEY, DB, RabbitMQ, GROQ_API_KEY, ACME_EMAIL, etc.
...
printf 'sk-...'          | docker secret create scsi_groq_key  -
```
...
- **Resumos de IA** preenchidos com **texto canônico fake** — o comando **não** chama a Groq (offline, gratuito, determinístico).
...
- **Jobs/Integrações:** nenhuma chamada à Groq/Celery; resumos são texto fake.
...
- [ ] Nenhuma chamada à Groq (resumos são texto fake).
...
> Este documento cresce, nunca diminui: novas features virão, novas subseções na seção de funcionalidades e novas sprints na seção 53, com a versão do cabeçalho incrementada.
